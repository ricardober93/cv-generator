import { getCurriculum, updateCurriculum } from "@/api/manager";
...
export const Route = createFileRoute("/_authenticated/edit/$id")({
  loader: async ({ params }) => {
    const data = await getCurriculum(params.id);
...
function Edit() {
  const data: Curriculum = Route.useLoaderData();
  ...
  const handleSaveCV = async () => {
    setIsLoading(true);
    await updateCurriculum(data.id, {
      name: state.name,
      email: state.email,
      phone: state.phone,
